    ...
    public partial class Form1 : Form
    {
        ...
        private void newToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (form.ShowDialog() == DialogResult.OK)
            {
                ...
                var winOrder = GameLoop();
            }
        }

        private List<Creature> GameLoop()
        {
            _creatures.Shuffle();
        ...
        private void DisplayCharacterStatus()
        {
            ...
            foreach (var creature in _creatures)
            {
                listBoxCharacterName.Items.Add($"{creature.CreatureName}");
                listBoxHealth.Items.Add($"{creature.CurrentHitPoints}");
                listBoxShields.Items.Add($"{creature.NumberOfShields}");
            }
        }
        ...
        private void PlayCard(Creature creature, Card specificCard = null)
        {
            if (creature.IsHuman && creature.CurrentHitPoints > 0)
            {
                ...
                if (specificCard != null)
                {
                    ...
                    listBoxHand.Items.Add(DisplayCard(specificCard));
                }
                else
                {
                    int optionNum = 1;
